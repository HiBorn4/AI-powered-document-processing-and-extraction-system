...
# image_path = 'data/JCAP/2584580.jpg'
input_folder = 'data/jcap/new_imgs'
output_folder = 'data/jcap/df_new_imgs'
...
def table_detection(image_path):
    ...
    if image_path.endswith('.jpg') or image_path.endswith('.png') or image_path.endswith('.jpeg'):
        ...
        if 'jcap':
            ...
            main_arr = jcap_tsr_main(image, table_bbox, ocr_model, image_path)
        elif 'jsw':
            img_table = image[int(y1):int(y2) , int(x1):int(x2)]
...
# main_arr = table_detection(image_path)
# print('main array')
# print(main_arr)
...
for file_name in os.listdir(input_folder):
    if file_name.endswith(('.jpg', '.png', '.jpeg')):
        ...
        print(image_path)
        data_frame = table_detection(image_path)
        print('data frame')
        print(data_frame)

        # # Save the overallData to a CSV file
        # print("file saved")
        ...
        # field_names=["ys","uts","el_perc", " RVALUNVALM,E", "ra"]
        # output_csv_path = os.path.join(output_folder, f'{os.path.splitext(file_name)[0]}.csv')
