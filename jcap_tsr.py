...
# import easyocr
from mypaddleocr.mypaddleocr import image_ocr

input_folder = 'data/JCAP/output'
output_folder = 'data/JCAP/data_frame'
image_path = 'data/JCAP/output/2584577_table.jpg'
          ...
          if (len(finalboxes[i][j]) == 0):
              ...
              for k in range(len(finalboxes[i][j])):
                  ...
                  if h==0 or w==0:
                    ...
                    resized = cv2.resize(final_crop, None, fx=1, fy=1, interpolation=cv2.INTER_CUBIC)
...
# def main(image, img_name, img_ext, outpath):
...
#     angle, deskew_crop_table_img = correct_skew(image)
...
#     img_line_removed = lines_removal(deskew_crop_table_img)    
...
#     table_dataframe = output_to_csv(finalboxes, deskew_crop_table_img)
...
def main(image, table_bbox, ocr_model):
    ...
    # TABLE OCR 
    print('OCRing the table image')
    table_dataframe = output_to_csv(finalboxes, img_line_removed)
...
#     #     print(image_path)
#         img = cv2.imread(image_path)
#         data_frame = main(img, os.path.splitext(file_name)[0], 'png', output_folder)
#         print('data frame')
#         print(data_frame)

#         # Save the ProcessedDataFrame to a CSV file
#         field_names=["ys","uts","el_perc","ra"]
